...
# make sure you rename RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us to a different variable that was not defined
...
# 3. while reading a supplied script and writing to location '/tmp/runscript2.sh'
# use appropriate directory ('tmp').
# again mount and check your disk image...
#################################################################################

# Test if the RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us environment variable is already set
echo "***** Start tux2 ckpt script! *****"
if [ "${RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us not set! So setting up network. Then reload self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us=1
else
	echo "Else part - RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us is set! So reload self and execute!"
	echo "Else part - Loading new script..."
	/sbin/m5 readfile > /tmp/runscript2.sh
	chmod 755 /tmp/runscript2.sh

	# Execute the new runscript
	if [ -s /tmp/runscript2.sh ]
	then
		#/system/bin/sh /data/runscript2.sh
		echo "Else part - executing newly loaded script ..."
		/bin/bash /tmp/runscript2.sh
...
echo "1. RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us is $RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us"
...
echo "2. RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us is $RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us"
if [ "$RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us=2
	echo "3. RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us is $RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES_50us"
