...
# make sure you rename RUNSCRIPT_VAR_NEHALEM_START_NPB to a different variable that was not defined
...
# Test if the RUNSCRIPT_VAR_NEHALEM_START_NPB environment variable is already set
echo "Test m5"
if [ "${RUNSCRIPT_VAR_NEHALEM_START_NPB+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR_NEHALEM_START_NPB not set! So setting it and taking checkpoint!"
	export RUNSCRIPT_VAR_NEHALEM_START_NPB=1
else
...
echo "1. RUNSCRIPT_VAR_NEHALEM_START_NPB is $RUNSCRIPT_VAR_NEHALEM_START_NPB"
...
echo "2. RUNSCRIPT_VAR_NEHALEM_START_NPB is $RUNSCRIPT_VAR_NEHALEM_START_NPB"
if [ "$RUNSCRIPT_VAR_NEHALEM_START_NPB" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR_NEHALEM_START_NPB=2
	echo "3. RUNSCRIPT_VAR_NEHALEM_START_NPB is $RUNSCRIPT_VAR_NEHALEM_START_NPB"
