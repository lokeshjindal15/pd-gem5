...
# make sure you rename RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES to a different variable that was not defined
...
# Test if the RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES environment variable is already set
echo "***** Start tux2 ckpt script! *****"
if [ "${RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES not set! So setting up network. Then reload self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES=1
else
	echo "Else part - RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES is set! So reload self and execute!"
...
echo "1. RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES is $RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES"
...
echo "2. RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES is $RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES"
if [ "$RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES=2
	echo "3. RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES is $RUNSCRIPT_VAR_TUX2_WORKING_PING_NPB_CKPT_5NODES"
