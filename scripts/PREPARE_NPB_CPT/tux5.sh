...
# make sure you rename RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES to a different variable that was not defined
...
# 3. while reading a supplied script and writing to location '/tmp/runscript5.sh'
# use appropriate directory ('tmp').
# again mount and check your disk image...
#################################################################################

# Test if the RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES environment variable is already set
echo "***** Start TUX5 ckpt script! *****"
if [ "${RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES not set! So setting up network. Then reload self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES=1
else
	echo "Else part - RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES is set! So reload self and execute!"
	echo "Else part - Loading new script..."
	/sbin/m5 readfile > /tmp/runscript5.sh
	chmod 755 /tmp/runscript5.sh

	# Execute the new runscript
	if [ -s /tmp/runscript5.sh ]
	then
		#/system/bin/sh /data/runscript5.sh
		echo "Else part - executing newly loaded script ..."
		/bin/bash /tmp/runscript5.sh
...
echo "1. RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES"
...
echo "2. RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES"
if [ "$RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES=2
	echo "3. RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX5_NPB_CKPT_16NODES"
