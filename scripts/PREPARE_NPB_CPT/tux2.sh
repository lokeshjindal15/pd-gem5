...
# make sure you rename RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES to a different variable that was not defined
...
# Test if the RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES environment variable is already set
echo "***** Start tux2 ckpt script! *****"
if [ "${RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES not set! So setting up network. Then reload self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES=1
else
	echo "Else part - RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES is set! So reload self and execute!"
...
echo "1. RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES"
...
echo "2. RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES"
if [ "$RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES=2
	echo "3. RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX2_NPB_CKPT_16NODES"
