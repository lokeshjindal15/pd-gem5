...
# make sure you rename RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES to a different variable that was not defined
...
# 3. while reading a supplied script and writing to location '/tmp/runscript7.sh'
# use appropriate directory ('tmp').
# again mount and check your disk image...
#################################################################################

# Test if the RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES environment variable is already set
echo "***** Start TUX7 ckpt script! *****"
if [ "${RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES not set! So setting up network. Then reload self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES=1
else
	echo "Else part - RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES is set! So reload self and execute!"
	echo "Else part - Loading new script..."
	/sbin/m5 readfile > /tmp/runscript7.sh
	chmod 755 /tmp/runscript7.sh

	# Execute the new runscript
	if [ -s /tmp/runscript7.sh ]
	then
		#/system/bin/sh /data/runscript7.sh
		echo "Else part - executing newly loaded script ..."
		/bin/bash /tmp/runscript7.sh
...
echo "1. RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES"

ifconfig eth0 10.0.0.9
ifconfig lo up
ifconfig eth0 hw ether 00:90:00:00:00:08
...
echo "2. RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES"
if [ "$RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES=2
	echo "3. RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES is $RUNSCRIPT_VAR_TUX7_NPB_CKPT_16NODES"
