...
# make sure you rename RUNSCRIPT_VAR_HASWELL_START_HADOOP to a different variable that was not defined
...
echo "I am inside haswell_start_hadoop.sh!"
# Test if the RUNSCRIPT_VAR_HASWELL_START_HADOOP environment variable is already set
echo "Test m5"
if [ "${RUNSCRIPT_VAR_HASWELL_START_HADOOP+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR_HASWELL_START_HADOOP not set! So setting it and taking checkpoint!"
	export RUNSCRIPT_VAR_HASWELL_START_HADOOP=1
else
...
echo "1. RUNSCRIPT_VAR_HASWELL_START_HADOOP is $RUNSCRIPT_VAR_HASWELL_START_HADOOP"
...
cat root/hadoop-1.0.2/logs/hadoop-root-namenode-tux0.log
echo "************************************************************************"
echo "3. Checkpointing simulation..."
/sbin/m5 checkpoint 0 0
#
#sleep 200
...
echo "2. RUNSCRIPT_VAR_HASWELL_START_HADOOP is $RUNSCRIPT_VAR_HASWELL_START_HADOOP"
if [ "$RUNSCRIPT_VAR_HASWELL_START_HADOOP" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR_HASWELL_START_HADOOP=2
	echo "3. RUNSCRIPT_VAR_HASWELL_START_HADOOP is $RUNSCRIPT_VAR_HASWELL_START_HADOOP"
